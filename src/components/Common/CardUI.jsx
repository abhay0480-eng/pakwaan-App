...
<section className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-5 max-w-screen-2xl mx-auto my-10'>
{
    resData?.map((restData,index)=>{
        return(
            ...
            <div>
                <p className='my-2 font-semibold '>{restData?.info?.name  || restData?.card?.card?.info?.name}</p>
                <p><span>{restData?.info?.avgRating || restData?.card?.card?.info?.avgRating}</span> <span>{restData?.info?.sla?.slaString ||  restData?.card?.card?.info?.sla?.slaString}</span></p>
